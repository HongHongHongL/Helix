...
__global__ void dev_const(__half *px, __half k) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  px[tid] = k;
}
...
__global__ void dev_iota(__half *px) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  px[tid] = tid;
}
...
void print(const __half *data, int n, int c, int h, int w) {
    std::vector<__half> buffer(1 << 20);
    CUDA_CALL(cudaMemcpy(
          buffer.data(), data,
          n * c * h * w * sizeof(__half),
          cudaMemcpyDeviceToHost));
    int a = 0;
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < c; ++j) {
...
int main(int argc, char *argv[]) {
  ...
  CUDNN_CALL(cudnnSetTensor4dDescriptor(
        in_desc, tensor_format, CUDNN_DATA_HALF,
  ...
  CUDNN_CALL(cudnnSetFilter4dDescriptor(
        filt_desc, CUDNN_DATA_HALF, tensor_format,
  ...
  CUDNN_CALL(cudnnSetTensor4dDescriptor(
        out_desc, tensor_format, CUDNN_DATA_HALF,
  ...
  __half *in_data, *filt_data, *out_data;

  CUDA_CALL(cudaMalloc(
         &in_data, in_n * in_c * in_h * in_w * sizeof(__half)));
        //&in_data, std::accumulate(in_dims.begin(), in_dims.end(), 1, multiplies<int>()) * sizeof(__half)));
  CUDA_CALL(cudaMalloc(
         &filt_data, filt_k * filt_c * filt_h * filt_w * sizeof(__half)));
      //&filt_data, std::accumulate(w_dims.begin(), w_dims.end(), 1, multiplies<int>()) * sizeof(__half)));
  CUDA_CALL(cudaMalloc(
        // &out_data, out_n * out_c * out_h * out_w * sizeof(float)));
        &out_data, out_n * out_c * out_h * out_w * sizeof(__half)));
