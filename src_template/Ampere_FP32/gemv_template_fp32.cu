...
void cpuF16F16Gemm(float *a, float *b, float *c, int M, int N, int K) {
...
//#define COL_NUM_COUNT (COL_THREAD * NUM_PER_THREAD) // 16 float
...
__global__ void mySGemvKernel_Stage4(const T *__restrict__ A, const T *__restrict__ B, T *__restrict__ C, size_t M, size_t N, size_t K){
    ...
    float tmp[params::N_TILE] = {0.0};
    int4 r_a[in_K_iters];
    int4 r_b[in_K_iters][N_TILE]; 
    size_t B_idx = (tid % COL_THREAD) * NUM_PER_THREAD + blockIdx.z * K; 
    size_t A_idx = (block_row + warp_row + tid_row) * K + (tid % COL_THREAD) * NUM_PER_THREAD; 
        ...
        for(size_t j = 0; j < (in_K_iters); ++j){
            for(size_t n = 0; n < N_TILE; ++n){
                ...
                for(size_t k = 0; k < NUM_PER_THREAD; ++k){
                    tmp[n] += (reinterpret_cast<float*>(&r_a[j]))[k] * (reinterpret_cast<float*>(&r_b[j][n]))[k];
                }
            }
        }
    }
    ...
    if (N_TILE % NUM_PER_THREAD == 0){
        *(int4*)(&C[C_idx * N + blockIdx.z * NUM_PER_THREAD]) = *(int4*)(&tmp[0]);
    }else{ 
        for(size_t n = 0; n < N_TILE; ++n)
        {
...
template<typename T, typename params>
void gemv(T* a, T* b, T* c, int M, int N, int K){
    dim3 block(params::THREADS_PER_BLOCK);
    dim3 grid(M / params::BLOCK_ROW, 1, div_ceil(N, params::N_TILE));
    mySGemvKernel_Stage4<T, params><<<grid, block>>>(a, b, c, M, N, K);
}

template<F16F16GemmTCAlgo_t algo = HGEMMAlignedV5>
void myF16F16GemmTCWarp(float *a, float *b, float *c, int M, int N, int K) {
    if(algo == HGEMMAlignedV5){
        constexpr int k_tile = 128;
        constexpr int num_per_thread = 4;
        constexpr int row_thread = 1;
        constexpr int warps_per_block = 4; 
        if(N <= 4){
            gemv<float, params<1, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
        }else if(N <= 8){
            if(N % 2 == 0){
                gemv<float, params<2, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
            }else{
                gemv<float, params<1, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
            }
        }else{
            if(N % 8 == 0){
                gemv<float, params<4, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
            }else if(N % 4 == 0){
                gemv<float, params<4, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
            }else if(N % 2 == 0){
                gemv<float, params<2, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
...
float testF16F16GemmMaxError(
    void (*gpuF16F16Gemm) (float *, float *, float *, int, int, int),
    int M, int N, int K) {
    ...
    srand(time(0));
    for (int i = 0; i < M * K; i++){
        h_a[i] = (float)(float(rand()) / float(RAND_MAX));
        //h_a[i] = (float)(float(i) / 100);
    }
    for (int i = 0; i < K * N; i++)
        //h_b[i] = (float)(rand() / float(RAND_MAX));
        h_b[i] = (float)(float(i)/ 10000);

    cpuF16F16Gemm(h_a, h_b, h_c, M, N, K);

    for (int i =0; i < N; ++i){
        for(int j = 0; j < K; ++j){
           h_b[i * K + j] = (float)((float)(j * N + i) / 10000);
        }
    }
