...
void myF16F16GemmTCWarp(half *a, half *b, half *c, int M, int N, int K) {
    if(algo == HGEMMAlignedV5){
        ...
        if(N <= 4){
            gemv<half, params<1, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
        }else if(N <= 8){
            if(N % 2 == 0){
                gemv<half, params<2, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
            }else{
                gemv<half, params<1, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
            }
        }else{
            if(N % 8 == 0){
                gemv<half, params<8, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
            }else if(N % 4 == 0){
                gemv<half, params<4, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
            }else if(N % 2 == 0){
                gemv<half, params<2, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
            }else{
                gemv<half, params<1, k_tile, num_per_thread, row_thread, warps_per_block>>(a, b, c, M, N, K);
