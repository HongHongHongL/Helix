...
from Ampere_FP16.Ampere_FP16_conv import get_Ampere_FP16_conv_Helix_result, get_Ampere_FP16_conv_cudnn_result
...
def Ampere_FP16_Helix_op_level_Conv_benchmark(backend = "cuda"):
    ...
    for batch, input_channel, H, W, output_channel, kH, kW, stride, pad in shape_list:
        _, helix_tflops = get_Ampere_FP16_conv_Helix_result(prof_dict, batch, input_channel, H, W, output_channel, kH, kW, stride, pad, backend)
        print(f'Helix: {helix_tflops:.2f} TFLOPS')

def Ampere_FP16_cudnn_op_level_Conv_benchmark():
    shape_list = get_conv_op_shape_list()
    for batch, input_channel, H, W, output_channel, kH, kW, stride, pad in shape_list:
        _, cudnn_tflops = get_Ampere_FP16_conv_cudnn_result(batch, input_channel, H, W, output_channel, kH, kW, stride, pad)
        print(f'cudnn: {cudnn_tflops:.2f} TFLOPS')

if __name__ == "__main__":

    Ampere_FP16_Helix_op_level_Conv_benchmark(backend="cuda")
    Ampere_FP16_Helix_op_level_Conv_benchmark(backend="tvm")
