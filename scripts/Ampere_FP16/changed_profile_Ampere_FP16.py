...
def profile_Helix_Ampere_FP16_gemm_kernel():
    ...
    with tqdm(total=(4*4*4+1)*2, desc="profiling") as pbar:
        for K_STAGE in range(2, 6):
            for WARP_ROWS in [16, 32, 64, 128]:
                for WARP_COLS in [16, 32, 64, 128]:
                    if not is_valid_config(1, 1, WARP_ROWS, WARP_COLS, WARP_ROWS, WARP_COLS):
                        pbar.update(2)
                        continue

                    cmd = f"{root_path}/build/bin_fp16/gemm_{K_STAGE}_{WARP_ROWS}_{WARP_COLS}_{WARP_ROWS}_{WARP_COLS} {WARP_ROWS} {WARP_COLS} 4096"
                    with os.popen(cmd) as result:
                        cost = result.read().split()[-8]
                        prof_dict[(K_STAGE, WARP_ROWS, WARP_COLS, WARP_ROWS, WARP_COLS, 0)] = float(cost)
                        for BLOCK_ROWS_SCALE in [1, 2, 4]:
                            for BLOCK_COLS_SCALE in [1, 2, 4]:
                                ...
                                if not is_valid_config(BLOCK_ROWS_SCALE, BLOCK_COLS_SCALE, BLOCK_ROWS, BLOCK_COLS, WARP_ROWS, WARP_COLS):
                                    continue
                                prof_dict[(K_STAGE, BLOCK_ROWS, BLOCK_COLS, WARP_ROWS, WARP_COLS, 0)] = float(cost) * BLOCK_ROWS_SCALE * BLOCK_COLS_SCALE
                        pbar.update(1)
                    
                    cmd_splitK = f"{root_path}/build/bin_fp16/gemm_splitK_{K_STAGE}_{WARP_ROWS}_{WARP_COLS}_{WARP_ROWS}_{WARP_COLS} {WARP_ROWS} {WARP_COLS} 4096"
                    with os.popen(cmd_splitK) as result_splitK:
                        cost_splitK = result_splitK.read().split()[-8]
                        prof_dict[(K_STAGE, WARP_ROWS, WARP_COLS, WARP_ROWS, WARP_COLS, 1)] = float(cost_splitK)
                        for BLOCK_ROWS_SCALE in [1, 2, 4]:
                            for BLOCK_COLS_SCALE in [1, 2, 4]:
                                BLOCK_ROWS = BLOCK_ROWS_SCALE * WARP_ROWS
                                BLOCK_COLS = BLOCK_COLS_SCALE * WARP_COLS
                                if not is_valid_config(BLOCK_ROWS_SCALE, BLOCK_COLS_SCALE, BLOCK_ROWS, BLOCK_COLS, WARP_ROWS, WARP_COLS):
                                    continue
                                prof_dict[(K_STAGE, BLOCK_ROWS, BLOCK_COLS, WARP_ROWS, WARP_COLS, 1)] = float(cost_splitK) * BLOCK_ROWS_SCALE * BLOCK_COLS_SCALE
                    pbar.update(1)
